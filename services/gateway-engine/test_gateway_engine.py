...
Or locally if deps are installed:
  pytest services/gateway-engine/test_gateway-engine.py -v
"""
...
async def test_post_with_retry_records_rate_limit_signal():
    ...
    req_before = _counter_value(
        t.PROVIDER_REQUESTS, provider="anthropic", model="claude-sonnet-4-6", outcome="rate_limited"
    )
    with patch.object(t, "_client", mock_client):
...
class TestGeminiReqToOai(unittest.TestCase):
    ...
    def test_customtools_suffix_stripped(self):
        oai = self._call(
            "gemini-3.1-pro-preview-customtools", gemini_map={"gemini-3.1-pro-preview": "gemini-3-1-pro-preview"}
        )
        assert oai["model"] == "gemini-3-1-pro-preview"

    def test_base_model_unchanged_when_no_suffix(self):
        oai = self._call("gemini-3.1-pro-preview", gemini_map={"gemini-3.1-pro-preview": "gemini-3-1-pro-preview"})
        assert oai["model"] == "gemini-3-1-pro-preview"
    ...
    def test_generic_tool_call_id_resolution_openai_format(self):
        history = [
            {"role": "user", "content": "hi"},
            {
                "role": "assistant",
                "tool_calls": [
                    {"id": "custom_call_999", "type": "function", "function": {"name": "read_file", "arguments": "{}"}}
                ],
            },
        ]
...
async def test_post_with_retry_retries_on_502():
    ...
    with patch.object(t, "_client", mock_client), patch("main.asyncio.sleep", new=AsyncMock()):
...
def test_responses_proxy_timeout_non_stream():
    ...
    with (
        patch("main._post_with_retry", mock_post_with_retry),
        patch.dict(os.environ, {"LITELLM_MASTER_KEY": "test-key"}),
    ):
        response = client.post(
            "/v1/responses", json={"model": "gpt-5.4", "messages": [{"role": "user", "content": "hello"}]}
        )
...
def test_responses_proxy_timeout_stream():
    ...
    with patch.object(t, "_client", mock_client), patch.dict(os.environ, {"LITELLM_MASTER_KEY": "test-key"}):
        response = client.post(
            "/v1/responses",
...
class TestAdminModelsPanel(unittest.TestCase):
    ...
    def test_drift_when_visible_not_configured(self):
        ...
        assert panel["status"] == "warning"
        assert any(
            d["model"] == "gemini-3-flash" and d["kind"] == "visible_not_configured" for d in panel["data"]["drift"]
        )
...
async def test_admin_status_endpoint_shape():
    ...
    with (
        patch.object(
            t, "_admin_load_litellm_config", return_value=({"model_list": [{"model_name": "claude-sonnet-4-6"}]}, [])
        ),
