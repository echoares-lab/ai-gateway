...
def test_virtual_provider_echoes_prompt(client, monkeypatch):
    ...
    body = {
        "model": "virt-claude-test",
        "messages": [
            {"role": "user", "content": "Hello Virtual World!"}
        ]
    }
...
def test_virtual_provider_simulates_429(client):
    body = {
        "model": "virt-error-429",
        "messages": [
            {"role": "user", "content": "Trigger limit"}
        ]
    }
...
def test_virtual_provider_disabled_by_default(monkeypatch):
    ...
    body = {
        "model": "virt-error-429",
        "messages": [{"role": "user", "content": "Trigger limit"}]
    }
