...
| `scripts/ops/` | Day-2 ops, env, mock seed, client bootstrap | `docker-cleanup.sh`, `sync-env-to-op.sh`, `push-rotated-secrets-to-op.sh`, `generate-staging-configmap.sh`, `load-mock-data.sh`, `generate-litellm-mock-seed.sh`, `gen-client-config.sh`, `setup-repo-env.sh`, `verify-docker-naming.sh`, `validate_dev_env_slots.py` |
