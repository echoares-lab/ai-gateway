...
| PostgreSQL | 17 | Runtime | Required by stack | `docker-compose.yml` | Persist gateway data | Use Compose service | `docker compose ps` |
...
| Make | unversioned | Project | Required | `Makefile` | Entry point for the repository gates (`make lint`, `make test-unit`, `make test-fast`) | Install make | `make -n lint` |

## 4. Developer tools

| Tool | Version | Category | Required | Source | Purpose | Installation | Verification |
|---|---|---|---|---|---|---|---|
| pytest | requirements/pyproject | Dev | Required | `pyproject.toml` | Unit/integration tests | `pip install -e .` | `pytest --version` |
| ruff | unversioned | Dev | Required | `Makefile` | Lint and format gate (`make lint`, mirrored by the CI fast tier) | `pip install ruff` | `ruff --version` |
...
| Tool | Version | Category | Required | Source | Purpose | Installation | Verification |
|---|---|---|---|---|---|---|---|
| Kubernetes | unversioned | Ops | Operator-only | `.github/workflows/promote-k3s-images.yml` | Deploy promoted gateway images (manifests live in the k3s-01 repo) | Cluster/operator managed | Deployment health check |
...
- `make lint` — ruff check and format check (source: `Makefile`; mirrored by the CI fast tier in `.github/workflows/ci.yml`).
- `make test-unit` — build the gateway-engine image and run the fully-mocked unit suite (source: `Makefile`).
- `make test-fast` — full fast tier: lint, unit, validators, drift, and mock tests (source: `Makefile`).

There is no bare-`make` default task and no `help` target; invoke a named target. Running
`make` with no argument falls through to the first rule, `clean-db`, which deletes Docker
volumes.
