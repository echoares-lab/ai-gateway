...
|----|--------------|-------------|--------|--------|
| P0-1 | Tenancy metadata in gateway-engine (`ak-*` → metadata) | [#30](https://github.com/echoares-lab/ai-gateway/issues/30) | **Done** ([#79](https://github.com/echoares-lab/ai-gateway/pull/79)) | 38-4, 38-5 |
...
**Status (2026-06-05):** P0-1..P0-3 are **Done**. Gateway Engine policy-engine wiring
(38-4) may proceed in dev; full Epic #38 Phase 1 remains blocked on P0-4..P0-7.

## Verification commands (when prerequisites land)

```bash
# P0-2: provider signals present in gateway-engine metrics
curl -s http://localhost:4000/metrics | grep gateway-engine_provider
