...
probe_gateway_engine_registry_models() {
...
PYEOF
    ...
    case "$probe_status" in
      ...
      missing_model)
        ...
        if gateway_engine_admin_patch "/admin/models/$model_id" '{"enabled":false,"status":"UNAVAILABLE","source":"sync-models"}'; then
          disabled=$((disabled + 1))
        else
          failed=$((failed + 1))
        fi
...
cmd_sync_models_registry() {
  ...
  if ! gateway_engine_admin_post "/admin/models/sync" '{"dry_run":false,"source":"cliproxy"}' "$sync_response"; then
  ...
  if [ "${CLIPROXY_SYNC_PROBE:-true}" != "false" ]; then
    probe_gateway_engine_registry_models "$sync_response"
  else
    echo ""
  ...
  if ! gateway_engine_admin_post "/admin/models/reconcile" '{"dry_run":true,"include_disabled":false}' "$reconcile_response"; then
