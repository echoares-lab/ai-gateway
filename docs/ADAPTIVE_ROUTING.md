...
Gemini models use the Antigravity credential pool inside CLIProxy. When that
pool is unavailable, LiteLLM falls back directly to a capability-equivalent
Claude model and then to the corresponding GPT tier. Deployment-specific
Gemini CLI aliases and credentials are not part of AI Gateway routing.
