...
GATEWAY_URL = os.environ.get("GATEWAY_URL", "http://localhost:4090")
...
def policy_debug():
    ...
    with httpx.Client(base_url=GATEWAY_URL, timeout=10) as client:
        try:
            resp = client.get("/debug/policy/last")
            if resp.status_code == 404:
                pytest.fail("translator mock policy debug not enabled (POLICY_MOCK_SCENARIOS?)")
        except httpx.ConnectError as exc:
            pytest.fail(f"translator not reachable at {GATEWAY_URL}: {exc}")
        yield client


@pytest.fixture(autouse=True)
def reset_policy_debug(policy_debug):
    policy_debug.post("/debug/policy/reset")
    yield
...
def _last_evaluate(policy_debug: httpx.Client) -> dict:
    payload = policy_debug.get("/debug/policy/last").json()
    assert payload, "translator received no in-process policy evaluate call"
    return payload


def _last_decision(policy_debug: httpx.Client) -> dict:
    payload = _last_evaluate(policy_debug)
    decision = payload.get("decision")
    assert isinstance(decision, dict), f"unexpected debug payload: {payload}"
    return decision


def _last_context(policy_debug: httpx.Client) -> dict:
    payload = _last_evaluate(policy_debug)
...
@pytest.mark.mock
def test_policy_engine_wired_in_mock_stack(client, policy_debug):
    resp = _chat(client, model="claude-sonnet-4-6")
    assert resp.status_code == 200, resp.text[:300]
    assert _last_decision(policy_debug).get("policy_version") == "mock-gate-b-v1"


@pytest.mark.mock
def test_agent_family_lock_blocks_cross_family_tools(client, policy_debug):
    resp = _chat(
        client,
    ...
    assert resp.status_code == 200, resp.text[:300]
    decision = _last_decision(policy_debug)
...
@pytest.mark.mock
def test_quota_429_preemptive_deprioritizes_credentials(client, policy_debug):
    resp = _chat(
        client,
        model="claude-sonnet-4-6",
...
@pytest.mark.mock
def test_quota_429_preemptive_from_translator_rate_limit_signals(client, policy_debug):
    seed = _chat(
        client,
        model="claude-sonnet-4-6",
    ...
    context = _last_context(policy_debug)
...
@pytest.mark.mock
def test_repo_allowlist_restricts_ordered_deployments(client, policy_debug):
    resp = _chat(client, model="claude-sonnet-4-6", metadata={"agent_id": "test:repo-allowlist"})
    assert resp.status_code == 200, resp.text[:300]
    assert _last_decision(policy_debug).get("ordered_deployments") == ["claude-sonnet-4-6"]


@pytest.mark.mock
def test_repo_denylist_gate(client, policy_debug):
...
@pytest.mark.mock
def test_budget_deny_hard_gate_with_retry_after(client, policy_debug):
...
@pytest.mark.mock
def test_cooldown_skip_removes_degraded_fallback(client, policy_debug):
...
@pytest.mark.mock
def test_inventory_exclude_deprioritizes_degraded_credentials(client, policy_debug):
    resp = _chat(
        client,
        model="claude-sonnet-4-6",
